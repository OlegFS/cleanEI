...
# Load Posterioes
Post = []
Eps = [0.05,0.2,0.5,0.7,0.8,0.92]#,0.5,0.8]#[0.05,0.1,0.2,0.3,0.5,0.65,0.8,0.95]
...
def get_activity(eps,sim_type = 'ML', plot=True,
    ...
    if plot:
        ...
        plt.show()
    return ST,sc,signal,bursts,dat
...
for i,eps in enumerate(Eps):
    res= get_activity(eps,sim_type='MAP', plot=False)
    mapData[:,i] = res[-1]
...
for i,eps in enumerate(Eps):
    ...
    ax[1].plot(xs_real,CVs[:,data_index],'o',markersize = 3)
ax[0].errorbar(origEps2,np.nanmean(IBIs,0),np.nanstd(IBIs,0))
ax[0].plot(Eps,mapData[0,:]/1000,'o')
#ax[0].plot(Eps,AllData[0,:,:].T/1000,'.r',markersize = 2)
ax[1].errorbar(origEps2,np.nanmean(CVs,0),np.nanstd(CVs,0))
ax[1].plot(Eps,mapData[1,:],'o')
